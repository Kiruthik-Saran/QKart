...
export const generateCartItemsFrom = (cartData, productsData) => {
  ...
  return cartData.map((cart) => {
    const data = productsData.filter(
      (product) => cart.productId === product._id
    )[0];
    console.log(data);
    return { ...data, ["qty"]: cart.qty };
  });
  //return arr;
};
...
const Cart = ({ products, items, handleQuantity, isReadOnly }) => {
  ...
  const handleAdd = (val, id) => {
    ...
    handleQuantity(val + 1, id);
  };
  const handleDelete = (val, id) => {
    //const val = event.target.value;
    handleQuantity(Math.max(val - 1, 0), id);
  };
  return (
    <>
      <Box className="cart">
            ...
            >
              ...
              >
                <ItemQuantity
                  ...
                  value={item.qty}
                  handleAdd={() => handleAdd(item.qty, item._id)}
                  handleDelete={() => handleDelete(item.qty, item._id)}
