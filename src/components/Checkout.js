...
const Checkout = () => {
  ...
  //const [selectedad, setadd] = useState("");
  ...
  const validateRequest = (items, addresses) => {
    ...
    if(localStorage.getItem("balance")<totalcost){
      ...
      enqueueSnackbar("Please add a new address Please select one shipping address to proceed",{variant:'warning'});
      return false;
    }
    else{
      localStorage.setItem("balance",localStorage.getItem("balance")-totalcost);
      return true;
    }
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Button startIcon={<CreditCard />} onClick={()=>performCheckout(token,items,addresses)} variant="contained">
              PLACE ORDER
            </Button>
          </Box>
        </Grid>
